...
import { showWinnerAnimation, showResults } from '../animations/endOfTournamentAnimation';
...
const TournamentTemplate = (props) => {
	...
	const { teamsInBasket, setTeam, matchTeams, setTeamsInBasket, resetTeams } = useContext(
		TeamsContext
	);
						...
						<Background flex className="tournament tournament-template">
							<TopBar>
								<StyledTitle>
									Turniej
									<br />
									<DateDescription>{moment(new Date()).locale('pl').calendar()}</DateDescription>
								</StyledTitle>
							</TopBar>
							<Wrapper>
								<MainDashboard className={'baskets-dashboard'}>
									...
									{tournament.length >= 0 && matchTeams.length > 0 ? <MatchResults /> : null}
									<PlayerBasketWrap>
										<PlayerBasket>
